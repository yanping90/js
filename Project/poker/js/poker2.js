$(function () {
	...
	//三张牌
	var cardsCheck = createCards([[3, 3], [4, 4], [5, 5], [6, 6], [7, 7]]).sort(userSort);
	...
	if (cardsCheck.length == 2) {
		...
		if (countArr.length == 2 && Math.max.apply(null, countArr) == 3) {
			...
			for(var i=0;i<cardsCheck.length-1;i++){
				if(cardsCheck[i].value = cardsCheck[i+1].value + 1){

				}
			}
		}
	} else if (cardsCheck.length == 6) {
		if(Math.max.apply(null,countArr) == 4){
			if(countArr.length == 3 || countArr.length == 2 ){
				console.log("true");
			} else {
				console.log("false");
			}
